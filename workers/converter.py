...
def convert_word_to_pdf(word_files, print_after_conversion=False, formatted=False):
    pdf_files = []
    for word_file in word_files:
        ...
        result = subprocess.run(['soffice', '--headless', '--convert-to', 'pdf', word_file_to_convert], capture_output=True)
        if result.returncode != 0:
            raise RuntimeError(f"Error converting {word_file} to PDF: {result.stderr}")
        
        # Move the converted file to the desired location
        converted_file = word_file.replace('.docx', '.pdf')
